# shp <- st_read(paste0(Sys.getenv("USERPROFILE"),"\\OneDrive - Oregon\\DEQ - Air Data Team - DataRepo\\Raw\\Geography\\census_tracts.shp"))
...
# Start here if you don't want to pull data again
DEQdf <- DEQdf4
  ...
  ggplot(DEQdf3, aes(Yr, mean, colour =  Project)) + 
    geom_point(size = 2) #+ facet_wrap(~Project)
  
  ggplot(DEQdf3, aes(Yr, CRisk, colour =  Project)) + 
    geom_point(size = 2) +
    ggtitle(paste("HAP \nCancer Risk"), subtitle = paste(yrfirst,"-",yrfinal)) +
  ylim(0,200) #+ facet_wrap(~Project)
  
  ggplot(DEQdf3, aes(Yr, NCRisk, colour =  Project)) + 
    geom_point(size = 2) +
    ggtitle(paste("HAP \nNonCancer Risk"), subtitle = paste(yrfirst,"-",yrfinal)) #+ facet_wrap(~Project)
